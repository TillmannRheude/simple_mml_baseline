...
class VisionTouch_Datamodule(pl.LightningDataModule):
    ...
    def __init__(self, 
        ...
        print("SPLIT NR: ", self.split_nr)

    def setup(self, stage=None):
        self.train_dataset = VisionTouch(split="train", zero_fill_rates=self.missing["missing_train"], variant=self.variant, split_nr=self.split_nr, seed=self.seed)
        self.val_dataset = VisionTouch(split="val", zero_fill_rates=self.missing["missing_valid"], variant=self.variant, split_nr=self.split_nr, seed=self.seed)
        self.test_dataset = VisionTouch(split="test", zero_fill_rates=self.missing["missing_test"], variant=self.variant, split_nr=self.split_nr, seed=self.seed)
