import os 
import torch
import torchcodec
import torchaudio
import pandas as pd

from transformers import AutoTokenizer
from torch.utils.data import Dataset

from codefiles.datasets.utils import create_missing_data_masks, apply_missing_mask

class CH_Sims(Dataset):
    def __init__(
        self,
        dataset_path: str = "/path/to/data/CHSIMS",
        split: str = "train",
        split_nr: int = 1,
        variant: str = "unimodal_1",
        zero_fill_rates: list = [0.0, 0.0],
        seed: int = 42,
        max_length: int = 128,
        take_every: int = 16,
        max_video_len: int = 32
    ) -> None:
        super().__init__()

        self.num_modalities = 3
        self.variant = variant
        self.datadir = dataset_path

        self.take_every = take_every
        self.max_video_len = max_video_len
        self.max_length = max_length

        df = pd.read_csv(os.path.join(dataset_path, "label_cv_splits.csv"))
        df = df[df[f"cv_split_{split_nr}"] == split].reset_index(drop=True)
        self.chsims_dataset = df

        self.folder_names = df["file_name"].tolist()
        self.file_names = df["id"].tolist()
        self.texts = df["text"].tolist()
        self.targets = df["unimodal_0"].tolist()

        self.ptfiles_dir = os.path.join(self.datadir, "ptfiles")
        os.makedirs(self.ptfiles_dir, exist_ok=True)

        # Precompute paths
        self.video_paths = []
        self.pt_paths = []
        for folder, fid in zip(self.folder_names, self.file_names):
            clip_id = str(fid).zfill(4)
            self.video_paths.append(os.path.join(self.datadir, folder, clip_id + ".mp4"))
            parent_dir = os.path.basename(folder)
            self.pt_paths.append(os.path.join(self.ptfiles_dir, f"{parent_dir}_{clip_id}.pt"))

        self.tokenizer = AutoTokenizer.from_pretrained("bert-base-chinese", use_fast=True)
        enc = self.tokenizer(
            self.texts,
            return_tensors="pt",
            padding="max_length",
            truncation=True,
            max_length=self.max_length,
        )
        self.input_ids_all = enc["input_ids"]          # (N, L)
        self.attn_mask_all = enc["attention_mask"]     # (N, L)

        # missing data
        self.zero_fill_masks, self.missing_stats = create_missing_data_masks(
            total_samples=len(df),
            num_modalities=self.num_modalities,
            missing_rates=zero_fill_rates,
            random_seed=seed,
        )

        self.orig_sr = 44100
        self.target_sr = 16000
        self.resampler = torchaudio.transforms.Resample(self.orig_sr, self.target_sr)

    def __len__(self):
        return len(self.chsims_dataset)
    
    def _to_audio_16k(self, audio):
        # audio: (C, N) or (N,)
        if audio.dim() == 2:
            audio = audio.mean(dim=0)  # mono (N,)

        # normalize integer PCM to [-1, 1] if needed
        if audio.dtype in (torch.int16, torch.int32, torch.int64, torch.uint8):
            audio = audio.to(torch.float32)
            audio = audio / 32768.0
        else:
            audio = audio.to(torch.float32)

        # resample to 16k
        audio_16k = self.resampler(audio)  # (T,)
        return audio_16k

    def __getitem__(self, idx):
        video_path = self.video_paths[idx]
        pt_path = self.pt_paths[idx]

        if os.path.exists(pt_path):
            pt_data = torch.load(pt_path)
            video = pt_data["video"]

            if "audio_16k" in pt_data:
                audio_16k = pt_data["audio_16k"]
            else:
                audio = pt_data["audio"]
                audio_16k = self._to_audio_16k(audio)
                pt_data["audio_16k"] = audio_16k
                torch.save(pt_data, pt_path)
        else:
            # Decode
            video_decoder = torchcodec.decoders.VideoDecoder(video_path)
            video = video_decoder[:].data  # (T, C, H, W)

            video = video[:: self.take_every]
            if video.shape[0] > self.max_video_len:
                video = video[: self.max_video_len]

            video = torch.nn.functional.interpolate(
                video.float(), size=(224, 224), mode="bilinear", align_corners=False
            )

            audio_decoder = torchcodec.decoders.AudioDecoder(video_path, sample_rate=44100, num_channels=1)
            audio = audio_decoder.get_all_samples().data  # typically (C, N)

            audio_16k = self._to_audio_16k(audio)
            torch.save({"video": video, "audio": audio, "audio_16k": audio_16k}, pt_path)

        input_ids = self.input_ids_all[idx].unsqueeze(0)
        attention_mask = self.attn_mask_all[idx].unsqueeze(0)

        # Missing data
        if "unimodal" not in self.variant:
            video = apply_missing_mask(video, self.zero_fill_masks[idx, 0])
            audio_16k = apply_missing_mask(audio_16k, self.zero_fill_masks[idx, 1])
            input_ids = apply_missing_mask(input_ids.float(), self.zero_fill_masks[idx, 2])
            attention_mask = apply_missing_mask(attention_mask.float(), self.zero_fill_masks[idx, 2])

        label = torch.tensor(self.targets[idx]).unsqueeze(0)

        return {
            "video": video,
            "audio": audio_16k,
            "input_ids": input_ids,
            "attention_mask": attention_mask,
            "label": label.long(),
        }

def collate_fn(batch):
    # video is already subsampled/capped in __getitem__
    audio_lens = [item["audio"].shape[-1] for item in batch]
    max_video_len = 32
    max_audio_len = max(audio_lens)

    # Pad videos (assume item['video'] is (T, C, H, W))
    padded_videos = []
    for item in batch:
        v = item["video"]
        pad_len = max_video_len - v.shape[0]
        if pad_len > 0:
            pad = torch.zeros((pad_len, v.shape[1], v.shape[2], v.shape[3]), dtype=v.dtype)
            v = torch.cat([v, pad], dim=0)
        else:
            v = v[:max_video_len]
        padded_videos.append(v)
    padded_videos = torch.stack(padded_videos, dim=0)

    # Pad audios
    audio_lens = [item["audio"].shape[-1] for item in batch]
    max_audio_len = max(audio_lens)

    padded_audio_16k = []
    for item in batch:
        a = item["audio"]  # (T,)
        pad_len = max_audio_len - a.shape[-1]
        if pad_len > 0:
            a = torch.cat([a, torch.zeros(pad_len, dtype=a.dtype)], dim=-1)
        padded_audio_16k.append(a)
    padded_audio_16k = torch.stack(padded_audio_16k, dim=0)  # (B, T)

    input_ids = torch.cat([item["input_ids"] for item in batch], dim=0)
    attention_mask = torch.cat([item["attention_mask"] for item in batch], dim=0)
    labels = torch.stack([item["label"] for item in batch], dim=0)
    text = torch.stack([input_ids, attention_mask], dim=1)

    return {
        "video": padded_videos,
        "audio": padded_audio_16k,
        "input_ids": input_ids,
        "attention_mask": attention_mask,
        "text": text,
        "label": labels,
    }








# older / experimental versions 
class CH_Sims_old(Dataset):
    def __init__(
        self,
        dataset_path: str = "/path/to/data/CHSIMS", 
        split: str = "train",
        split_nr: int = 1, 
        variant: str = "unimodal_1",
        zero_fill_rates: list = [0.0, 0.0],
        seed: int = 42
    ) -> None: 
        super().__init__()

        self.num_modalities = 3
        self.variant = variant

        self.datadir = dataset_path
        self.chsims_dataset = pd.read_csv(os.path.join(dataset_path, "label_cv_splits.csv"))
        self.chsims_dataset = self.chsims_dataset[self.chsims_dataset[f"cv_split_{split_nr}"] == split].reset_index(drop=True)

        self.folder_names = self.chsims_dataset.loc[:, "file_name"]
        self.file_names = self.chsims_dataset.loc[:, "id"]
        self.texts = self.chsims_dataset.loc[:, "text"]
        self.targets = self.chsims_dataset.loc[:, "unimodal_0"]

        self.tokenizer = BertTokenizer.from_pretrained('bert-base-chinese')

        # missing data
        self.zero_fill_masks, self.missing_stats = create_missing_data_masks(
            total_samples=len(self.chsims_dataset),
            num_modalities=self.num_modalities,
            missing_rates=zero_fill_rates,
            random_seed=seed
        )

        # Create ptfiles directory if it does not exist
        self.ptfiles_dir = os.path.join(self.datadir, "ptfiles")
        if not os.path.exists(self.ptfiles_dir):
            os.makedirs(self.ptfiles_dir, exist_ok=True)

    def __len__(self):
        return len(self.chsims_dataset)

    def __getitem__(self, idx):
        video_path = os.path.join(self.datadir, self.folder_names[idx], str(self.file_names[idx]).zfill(4) + ".mp4")

        # Build pt file name from parent directory and file (e.g., video_0001_0025.pt)
        parent_dir = os.path.basename(self.folder_names[idx])
        clip_id = str(self.file_names[idx]).zfill(4)
        pt_filename = f"{parent_dir}_{clip_id}.pt"
        pt_path = os.path.join(self.ptfiles_dir, pt_filename)
        if os.path.exists(pt_path):
            pt_data = torch.load(pt_path)
            video = pt_data["video"]
            audio = pt_data["audio"]
        else:
            # Video
            video_decoder = torchcodec.decoders.VideoDecoder(video_path)
            video_frames = video_decoder[:]
            video = video_frames.data  # (T, C, H, W)
            # Video processing
            video = torch.nn.functional.interpolate(
                video.float(), size=(224, 224), mode='bilinear', align_corners=False
            )

            # Audio
            audio_decoder = torchcodec.decoders.AudioDecoder(video_path, sample_rate=44100, num_channels=1)
            audio_samples = audio_decoder.get_all_samples()
            audio = audio_samples.data

            # Save both in .pt for later convenience
            torch.save({"video": video, "audio": audio}, pt_path)

        # Text 
        text = self.texts[idx]
        # Text processing 
        encoding = self.tokenizer(
            text,
            return_tensors='pt',
            padding='max_length',
            truncation=True,
            max_length=128
        )
        input_ids = encoding['input_ids']
        attention_mask = encoding['attention_mask']

        # Missing data
        if not "unimodal" in self.variant:
            video = apply_missing_mask(video, self.zero_fill_masks[idx, 0])
            audio = apply_missing_mask(audio, self.zero_fill_masks[idx, 1])
            input_ids = apply_missing_mask(input_ids.float(), self.zero_fill_masks[idx, 2])
            attention_mask = apply_missing_mask(attention_mask.float(), self.zero_fill_masks[idx, 2])

        # Get label
        label = self.targets[idx]
        label = torch.tensor(label).unsqueeze(0)

        return {
            'video': video,                
            'audio': audio,
            'input_ids': input_ids,
            'attention_mask': attention_mask,
            'label': label.long()
        }

def collate_fn_old(batch):
    # Find max video length (T) and max audio length (num_samples)
    audio_lens = [item['audio'].shape[-1] for item in batch]
    max_video_len = 32  # 500 / 16 = 32
    max_audio_len = max(audio_lens)

    # Pad videos
    padded_videos = []
    for item in batch:
        v = item['video']  # (T, C, H, W)
        # only take every x-th frame
        v = v[::16, ...]
        pad_len = max_video_len - v.shape[0]
        if pad_len > 0:
            pad = torch.zeros((pad_len, v.shape[1], v.shape[2], v.shape[3]), dtype=v.dtype, device=v.device)
            v = torch.cat([v, pad], dim=0)
        padded_videos.append(v)
    padded_videos = torch.stack(padded_videos, dim=0)  # (B, T, C, H, W)

    # Pad audios
    padded_audios = []
    for item in batch:
        a = item['audio']  # (num_channels, num_samples)
        pad_len = max_audio_len - a.shape[-1]
        if pad_len > 0:
            pad = torch.zeros((a.shape[0], pad_len), dtype=a.dtype, device=a.device)
            a = torch.cat([a, pad], dim=-1)
        padded_audios.append(a)
    padded_audios = torch.stack(padded_audios, dim=0)  # (B, num_channels, num_samples)

    # Stack input_ids and attention_mask (already padded to max_length=128)
    input_ids = torch.cat([item['input_ids'] for item in batch], dim=0)  # (B, L)
    attention_mask = torch.cat([item['attention_mask'] for item in batch], dim=0)  # (B, L)

    # Stack labels
    labels = torch.stack([item['label'] for item in batch], dim=0)

    # Stack text
    text = torch.stack([input_ids, attention_mask], dim=1)

    return {
        'video': padded_videos,
        'audio': padded_audios,
        'input_ids': input_ids,
        'attention_mask': attention_mask,
        'text': text,
        'label': labels,
    }