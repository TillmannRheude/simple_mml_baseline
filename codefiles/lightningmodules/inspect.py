...
class INSPECT_Lightning_Module(LightningModuleParent):
    ...
    def __init__(
        ...
        self.binary_mces = [BinaryCalibrationError(norm="max", ignore_index=self.ignore_index) for _ in range(1)]
        self.binary_rmsces = [BinaryCalibrationError(norm="l2", ignore_index=self.ignore_index) for _ in range(1)]
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.metrics_detailed[0].update(shared_dict["logits"].view(-1, 1), shared_dict["y"].view(-1, 1))
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.metrics_detailed[2].update(shared_dict["logits"].view(-1, 1), shared_dict["y"].view(-1, 1))
        for i in range(1):
            shared_dict["y"][torch.isnan(shared_dict["y"])] = self.ignore_index
            logits = shared_dict["logits"].to(torch.float32)
            y = shared_dict["y"].to(torch.float32)
    ...
    def on_validation_epoch_end(self):
        ...
        if not self.trainer.sanity_checking:
            ...
            binary_eces = [self.binary_eces[i].compute() for i in range(1)]
            binary_mces = [self.binary_mces[i].compute() for i in range(1)]
            binary_rmsces = [self.binary_rmsces[i].compute() for i in range(1)]
            for i in range(1):
    ...
    def on_test_epoch_end(self):
        ...
        binary_rmsces = [self.binary_rmsces_test[i].compute() for i in range(1)]
        for i in range(1):
