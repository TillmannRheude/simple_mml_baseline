...
class MMPareto_Transformer(nn.Module):
    ...
    def forward(
        ...
        unimodal_feature_sequences = list(torch.split(x, split_size, dim=1))
        
        # Unimodal logits
        unimodal_logits = []
        for i, features in enumerate(unimodal_feature_sequences):
            pooled_features = torch.mean(features, dim=1)
            unimodal_logits.append(self.unimodal_heads[i](pooled_features))
