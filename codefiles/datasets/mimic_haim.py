...
class MIMIC_Haim(Dataset):
    def __init__(
        ...
        # targets
        self.targets = self.haim_dataset.loc[:, self.clf_labels]
...
class MIMIC_Haim_preloaded(Dataset):
    def __init__(
        self, 
        dataset_path: str = "/sc-resources/dh-mimic/mimic_haim/haim_img_report_lab_target_split.csv",
        split: str = "train",
        ...
        # image, lab paths
        img_paths = [f"{self.root_img}/{self.haim_dataset.at[i, 'img_folder']}/{self.haim_dataset.at[i, 'img_id']}.jpg" for i in range(len(self.haim_dataset))]
        img_paths = [path.replace('files', 'files_resized') for path in img_paths]
        lab_paths = [f"{self.root_lab}/laboratory_for_images/tensors/{self.haim_dataset.at[i, 'laboratory_filename']}.pt" for i in range(len(self.haim_dataset))]
        self.max_first_dim = 358  # for uniform sequence length lab values
        
        # Pre-load data to RAM to speed up training
        print(f"Pre-loading {split} images into memory...")
        self.images = [torch.from_numpy(cv2.imread(path, cv2.IMREAD_GRAYSCALE)).float().unsqueeze(0) for path in img_paths]
        print(f"Pre-loading {split} lab values into memory...")
        self.labs = [torch.load(path, weights_only=True) for path in lab_paths]
